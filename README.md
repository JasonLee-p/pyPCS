...
This project aims to creat a powerfull tool for complex music analysing and tiny pieces' generating (based on random generating then screening)
...
There are two ways to initialize a object：

```sh
# 1.Assign a value directly：
segment = pyPCS.PitchSegment([[60,65,58,69], ['1','1','3/2','1/2']])
# The argument is a list，inside witch the first arg is note list and the second one is a dutation list.
# 
# 2.Creat an empty pitchsegment and then randomly assign it：（Attention: Func new_segment is available only when the object's segment is empty.）
"""
segment = pyPCS.PitchSegment(None)
segment.new_segment(
  note_num=4,  # Number of notes
  total_duration=4,  # Total dutation (beats)
  first_note=60,  # The first note's value
  note_scale=4,  # Standard diviation of generated note values
  style='smooth')  # The style of rhyme (Now there are three styles.)"""
```

——Attention：In order to make this module compatible to module mido，we defined C4 as 60 and so on for the other notes. For example, G4 is 67.

——The length of note list should be equal to duration list，and larger than 2。

——In order to maintain the accuracy of calculations，the elements in duration list should be string variable, like '1/2', '4/3', '1'.
...
```sh
# Use the "+" operator to shift：
segment_T1, original_segment, transform_type = segment + (-1)
# Use method _transposition() (Not recommended)
segment_T1, original_segment, transform_type = segment._transposition(-1)
...
# Use method _retrograde_with_rhyme()
segment_R, original_segment, transform_type = segment._retrograde_with_rhyme()
...
```sh
print(egment._retrograde_with_rhyme(3))
```
Output：
```sh
...
It returns three vars: the new object after shifting, the parent object, and method of the transform.
```sh
# Using method _retrograde_without_rhyme()
segment_R, original_segment, transform_type = segment._retrograde_without_rhyme()
print(segment_R.segment)
print(original_segment.segment)
print(transform_type)
```
...
Pass an int argument to the function，and it will do transposition after retrograding.
```sh
print(egment._retrograde_without_rhyme(3))
```
Output：
```sh
...
## （一）类 PitchSegment
意为音高截段，相关名词可以参考文献《Introduction to Post-Tonal Theory》（《后调性理论导论》）
...
该方法返回三个值：移位后的新实例，父实例，转换方法
```sh
# 使用内置方法_retrograde_without_rhyme()
segment_R, original_segment, transform_type = segment._retrograde_without_rhyme()
print(segment_R.segment)
print(original_segment.segment)
print(transform_type)
