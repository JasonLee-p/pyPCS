...
# from pyPCS._player import play_pitch_segment
import time

import numpy as np
from pyPCS._basicData import _prt_func_time, _prt_funcs_time, _prt_func_run_num
        ...
        if style == "smooth":
            ...
            for j in range(note_num - 1):
                if note_num < total_duration:
                    ...
                    rhythm_set.append(n) if n > 0 else rhythm_set.append(1)
                if note_num > total_duration:
                    n = find_nearest(smooth_ad, np.random.normal(loc=avr_dur, scale=avr_dur, size=None))
                    rhythm_set.append(n) if n > 0 else rhythm_set.append(0.5)

        elif style == "more doted notes":
            for j in range(note_num - 1):
                if j != 0:  # 模拟附点音符
                    if rhythm_set[-1] == 1.5 and avr_dur > 0.75:
                        rhythm_set.append(0.5)
                        continue
                    if rhythm_set[-1] == 0.75 and avr_dur > 0.37:
                        rhythm_set.append(0.25)
                        continue
                n = find_nearest(m_do_n_ad, np.random.normal(loc=avr_dur, scale=avr_dur, size=None))
                rhythm_set.append(n) if n > 0 else rhythm_set.append(0.25)
            for j in range(1, 5):
            ...
            for j in range(note_num - 1):
                ...
                rhythm_set.append(n) if n > 0 else rhythm_set.append(1 / 3)
                # print(n)

        rest_duration = round(total_duration - sum(rhythm_set), 2)
        if sum(rhythm_set) < total_duration and rest_duration in all_available_durations:
            flag = False
        else:
            print("Failed: rest duration:" + str(rest_duration))
    rhythm_set.append(total_duration - sum(rhythm_set))
    return rhythm_set
